...
\end{enumerate}
We chose this design over the two main alternatives deliberately. Full inverse rendering \cite{irgs} would model the footprint physically but constrains scene materials and lighting and is expensive to fit; iterative edit-and-optimise loops \cite{in2n,igs2gs} interleave many partial edits with 3D updates to enforce consistency, at several times the compute. Render--edit--refit is the cheapest member of this family --- one edit, one re-fit --- and part of our experimental question is precisely whether the expensive machinery is needed (Section~\ref{sec:consistency}). The pipeline trains no networks and needs no COLMAP: poses come from the orbit renderer.
...
\begin{itemize}
  \item \textbf{Polished dielectric floor, not a metallic mirror floor.} Our first floor was metallic: it produced a beautiful reflection and \emph{no visible shadow}, because a pure specular surface has no diffuse response for a shadow to darken. A glossy dielectric floor (roughness 0.04) shows both a soft reflection and a clear shadow.
  \item \textbf{A separate vertical mirror panel} (metallic, roughness 0.01). At our viewing angles, dielectric Fresnel reflection on the floor is too weak to serve as the headline mirror effect; a dedicated mirror gives the footprint a second, unambiguous component.
  ...
  \item \textbf{$832{\times}480$, 81 frames} matches the native resolution of the video models we evaluate and their $16n{+}1$ frame-count constraint, so the editor never resamples our data.
...
\label{sec:masks}
The editors edit only inside the mask, so mask coverage is a first-class experimental variable. Three protocols, stated precisely:
\begin{itemize}
